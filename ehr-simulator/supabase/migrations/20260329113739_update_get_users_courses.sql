  ...
  assigned_per_course AS (
    SELECT
      ...
      jsonb_agg(DISTINCT jsonb_build_object(
        'id',          sa.id,
        'case_id',     sa.case_id,             -- NEW
        'session_id',  cs.id,                  -- NEW
        'name',        cd.name,
    ...
    -- NEW: Join case_sessions to find the active session for this specific group
    LEFT JOIN public.case_sessions cs ON cs.case_id = sa.case_id 
