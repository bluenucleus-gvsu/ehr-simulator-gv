...
-- Enable documentation upsert by time offset and case session id
ALTER TABLE IF EXISTS editable_documentation_results 
  ADD CONSTRAINT unique_session_time UNIQUE (case_session_id, time_offset);

ALTER TABLE IF EXISTS editable_documentation_results 
  ADD COLUMN spo2_source text,
  ADD COLUMN pain_location text,
  ADD COLUMN pain_characteristics text,
  ADD COLUMN pain_alleviating_factors text,
  ADD COLUMN pain_aggravating_factors text,
  ADD COLUMN pain_interventions text,
  ADD COLUMN urine_description text;
